...
//GC-Wii related stuff
	...
	#define HW_NINTENDO
	#define MERITOUS_VERSION "v 1.2 GC"
	#define FILES_PATH		"/data/meritous/"
#elif HW_RVL
	#include <unistd.h>
	#include <gccore.h>
	...
	#define HW_NINTENDO
	#define MERITOUS_VERSION "v 1.2 Wii"
	#define FILES_PATH		"/data/meritous/"
#else
...
/*			if (!strcasecmp(argv[i], "record")) {
...
#ifdef HW_NINTENDO
	SDL_ShowCursor(SDL_DISABLE);//we don't need the mouse
	//on the wii the first read will init the default device to root, then we chdir and read again
	asceai = IMG_Load("dat/i/asceai.png");
	chdir(FILES_PATH);//get to where the files are
#endif
	...
	#ifndef HW_NINTENDO //for some reason this code crashes wii's event exception handler
			if ((tick % 10) == 0) {
				for (i = 0; i < 640*480; i++) {
...
void HandleEvents()
{
	...
	u16 GC_down = PAD_ButtonsDown(0);
		...
		if (GC_down & PAD_BUTTON_LEFT
		#ifdef HW_RVL
			|| WM_down & WPAD_BUTTON_LEFT || WM_down & WPAD_CLASSIC_BUTTON_LEFT
		#endif			
			)
			{
				key_held[K_LT] = 1;
		...
		if (GC_down & PAD_BUTTON_RIGHT
		#ifdef HW_RVL
			|| WM_down & WPAD_BUTTON_RIGHT || WM_down & WPAD_CLASSIC_BUTTON_RIGHT
		#endif			
			)
			{
				key_held[K_RT] = 1;
		...
		#ifdef HW_RVL
			|| WM_down & WPAD_BUTTON_B || WM_down & WPAD_CLASSIC_BUTTON_B
		#endif			
			)
			{
				key_held[K_SP] = 1;
		...
		#ifdef HW_RVL
			|| WM_down & WPAD_BUTTON_A || WM_down & WPAD_CLASSIC_BUTTON_A
		#endif			
			)
			{
				enter_pressed = 1;
		...
		if (GC_down & PAD_BUTTON_Y
		#ifdef HW_RVL
			|| WM_down & WPAD_BUTTON_MINUS || WM_down & WPAD_CLASSIC_BUTTON_X
		#endif			
			)
			{
				if (tele_select) {
		...
		if (GC_down & PAD_TRIGGER_Z
		#ifdef HW_RVL
			|| WM_down & WPAD_BUTTON_1 || WM_down & WPAD_CLASSIC_BUTTON_MINUS
		#endif			
			)
			{
				CancelVoluntaryExit();
		...
		if (GC_down & PAD_BUTTON_X
		#ifdef HW_RVL
			|| WM_down & WPAD_BUTTON_PLUS || WM_down & WPAD_CLASSIC_BUTTON_PLUS
		#endif			
			)
			{
				game_paused ^= 1;
		...
		if ( (GC_down & PAD_TRIGGER_L && GC_down & PAD_TRIGGER_R)
		#ifdef HW_RVL
			|| (WM_down & WPAD_CLASSIC_BUTTON_ZL && WM_down & WPAD_CLASSIC_BUTTON_ZR)
		#endif
			)
			{
				player_shield = 20;
		...
		/*if (WM_down & WPAD_CLASSIC_BUTTON_FULL_L)
			{
				int i, n, j;
				for (j = 0; j < 1; j++) {
					for (i = 0; i < 50000; i++) {
		...
		if (WM_down & WPAD_CLASSIC_BUTTON_ZL)
			{
				int i;
				for (i = 0; i < 8; i++) {
					artifacts[i] = 1;
		...
		if (WM_down & WPAD_CLASSIC_BUTTON_ZR)
			{
				current_boss = 3;
				expired_ms = 1000000;
			}
		*/
			...
			if (GC_up & PAD_BUTTON_DOWN
				...
				key_held[K_DN] = 0;
			if (GC_up & PAD_BUTTON_LEFT
			#ifdef HW_RVL
				|| WM_up & WPAD_BUTTON_LEFT || WM_up & WPAD_CLASSIC_BUTTON_LEFT
			#endif
				)
				key_held[K_LT] = 0;
			if (GC_up & PAD_BUTTON_RIGHT
			#ifdef HW_RVL
				|| WM_up & WPAD_BUTTON_RIGHT || WM_up & WPAD_CLASSIC_BUTTON_RIGHT
			#endif
				)
				key_held[K_RT] = 0;
			if (GC_up & PAD_BUTTON_A
			#ifdef HW_RVL
				|| WM_up & WPAD_BUTTON_B || WM_up & WPAD_CLASSIC_BUTTON_Y
			#endif
				)
				key_held[K_SP] = 0;
#endif	
